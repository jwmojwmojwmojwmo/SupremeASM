...
Instructions are 4/8 bytes and written in hex. However, print instructions will print in base 10.
 ...
 Anything in memory is stored as ints (4 bytes). One memory slot is equal to 4 bytes. Memory is allocated by the compiler implicitly for instructions. Memory must be explicitly allocated by instructions to store data, and deallocated if needed. Memory is allocated as blocks, and can only be deallocated block by block. 
